...
//	def defineNewArrayType ( size : Int , dataType : Int ) : Int
	...
	def cleanup (){
		buffersToDelete synchronized {
			var remaining = 10
			while ( buffersToDelete.nonEmpty && remaining > 0 ){
				glDeleteBuffers(buffersToDelete.last)
				buffersToDelete = buffersToDelete.dropRight(1)
				remaining -= 1
			}
		}
	}
...
class VBO(activeArrays_arg:Int) extends TVBO {
	...
	def bind () {
		if ( ! GL.disabled ) {
			if ( ! Application.isOpenGLThread ) { throw new IllegalStateException("Calling VBO bind on non-GL thread") }
			if ( VBO.boundArrayBuffer != points.name ) {
				VBO.bindBuffer(GL_ARRAY_BUFFER,points.name)
			}
			if ( VBO.boundIndexBuffer != indices.name ) {
				VBO.bindBuffer(GL_ELEMENT_ARRAY_BUFFER,indices.name)
			}
		}
	}
...
//		}
	...
	def isEmpty = (numPoints == 0 && numIndices == 0 && ! isSolidified) || (points.numSolidifiedElements == 0 && indices.numSolidifiedElements == 0 && isSolidified)
